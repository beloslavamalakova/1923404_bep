...
def prepare_data(train, test, lag_cols, static_cols, target_col="target"):
    ...
    X_train_seq = seq_scaler.fit_transform(X_train_seq).reshape(-1, len(lag_cols), 1)
    X_test_seq = seq_scaler.transform(X_test_seq).reshape(-1, len(lag_cols), 1)
...
def train_one_model(model_name, feature_set_name, train, test, lag_cols, static_cols, args, device):
    ...
    for epoch in range(args.epochs):
        ...
        if args.verbose:
            print(f"{model_name} | {feature_set_name} | epoch {epoch+1:02d}/{args.epochs} | loss {np.mean(losses):.4f}")
